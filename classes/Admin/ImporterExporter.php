...
class ImporterExporter {
	...
	public static function form_export(): void {
        ...
        <form method="post">
            ...
            <p>
				<?php
				submit_button( __( 'Export All Data', 'mwp-herd-effect' ), 'secondary', 'submit', false ); ?><?php
        ...
        <form method="post" enctype="multipart/form-data" action="">
            ...
            <p>
                <label>
					...
					<?php esc_html_e( 'Update item if item already exists.', 'mwp-herd-effect' ); ?>
                </label>
            ...
            <p>
				<?php
				submit_button( __( 'Import', 'mwp-herd-effect' ), 'secondary', 'submit', false ); ?><?php
	...
	public static function import_data(): void {
		...
		if ( ! isset( $_FILES['import_file'] ) || empty( $_FILES['import_file']['name'] ) ) {
			wp_die( esc_attr__( 'Please select a file to import', 'mwp-herd-effect' ),
				esc_attr__( 'Error', 'mwp-herd-effect' ),
				[ 'response' => 400 ] );
		}

        if ( self::get_file_extension( sanitize_text_field( $_FILES['import_file']['name'] ) ) !== 'json' ) {
			wp_die(
				esc_html__( 'Please upload a valid .json file', 'mwp-herd-effect' ),
				esc_html__( 'Error', 'mwp-herd-effect' ),
				[ 'response' => 400 ] );
		}
